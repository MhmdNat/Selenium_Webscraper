...
PATH = "C:\\Users\\user 2\\Desktop\\WebScraping Stuff\\chromedriver.exe" # CHANGE FOR YOUR SPECIFIC DIRECTORY
service = Service(PATH)
wd = webdriver.Chrome(service=service)


def get_urls_from_google(wd, delay, url, left):
    ...
    while True:
        ...
        for img in thumbnails[len(image_urls) : ]:
            try:
                ...
                continue
        if len(thumbnails)==96 or len(thumbnails) == left: # stop when all the page has been read
            return image_urls
...
def main():
    plant_name_ids = species_name_to_id() #returns list of tuples (name, id)
    ...
    for name, ID in plant_name_ids: #for the tuple returned by species_name_to_id
        ...
        print("num_full:",number_of_full_pages)
        thumbnails_left = total_thumbnails % 96
        print("left: ",thumbnails_left)
        ...
        for i in range(1,number_of_full_pages+1):
            ...
            urls.update(get_urls_from_google(wd, 3, page_url, left = -1)) # get image urls and add them to our url set
            
        if thumbnails_left:
            page_url = f"https://www.inaturalist.org/observations?page={number_of_full_pages+1}quality_grade=research&taxon_id={ID}"
            urls.update(get_urls_from_google(wd, 3, page_url, left = thumbnails_left))
